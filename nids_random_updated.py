...
# 1. Load Scikit-Learn Models via Dynamic Path Resolver
...
# 2. Try loading TensorFlow models if available
cnn_bin, cnn_multi, lstm_bin, lstm_multi = None, None, None, None
HAS_TF = False
...
# 3. Load Validation Dataset & Sample 1 Random Row
...
sample_features = sample_row.iloc[0, 0:16].values
true_attack = str(sample_row.iloc[0, 16]).strip().lower() if sample_row.shape[1] >= 17 else 'normal'

# Encode and scale
encoded_vec = prep.encode_feature_vector(sample_features)
...
knn_desc = prep.ATTACK_DESCRIPTIONS.get(knn_multi_pred, 'This Is Safe.') if knn_bin_pred == 1 else 'This Is Safe.'
...
rf_desc = prep.ATTACK_DESCRIPTIONS.get(rf_multi_pred, 'This Is Safe.') if rf_bin_pred == 1 else 'This Is Safe.'
...
# 7. Print legacy stdout lines
print(f"KNN Binary Class Type : {knn_bin_label}")
print(f"KNN Multi Class Type : {knn_multi_label}")
print(f"KNN Description : {knn_desc}")

print(f"RANDOM FOREST Binary Class Type : {rf_bin_label}")
print(f"RANDOM FOREST Multi Class Type : {rf_multi_label}")
print(f"RANDOM FOREST Description : {rf_desc}")

print(f"CNN Binary Class Type : {cnn_bin_label}")
print(f"CNN Multi Class Type : {cnn_multi_label}")
print(f"CNN Description : {cnn_desc}")

print(f"LSTM Binary Class Type : {lstm_bin_label}")
print(f"LSTM Multi Class Type : {lstm_multi_label}")
print(f"LSTM Description : {lstm_desc}")

# 8. Output Structured JSON Payload (Line 13)
result_json = {
    ...
    "sample_input": {prep.SELECTED_FEATURES_16[i]: str(sample_features[i]) for i in range(16)}
}
